...
The user authorized continuing body publication on 2026-09-23 and later requested outward media releases of at least twenty papers per batch. Human-reviewed packets should therefore normally accumulate about twenty current-corpus papers before a publication release when sufficient backlog exists; a small stable remainder at the end does not need an artificial twentieth paper; the immutable approval files may still be split internally into five-paper/thirty-image envelopes. Prioritize recent papers and JACS among otherwise equal candidates, then waiting backlog. This media maintenance is not a replacement literature task and is not a claim that the assistant watches the desktop or repairs code continuously.
