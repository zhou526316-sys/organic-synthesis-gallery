...
Each immutable envelope contains `schemaVersion:1`, a unique `batchId`, `mediaGeneration:1790082000000`, `approvedCount` matching the array, and `items`. For audit safety an individual envelope remains capped at five papers/thirty images, but one user-visible publication packet may contain multiple envelopes. The normal outward publication target is twenty to twenty-five articles when enough eligible media exists. A final or temporarily sparse tail below twenty may be released after fifteen minutes with no new qualifying TOC/body evidence, so media is not held indefinitely just to satisfy an artificial batch size. An item has the same verified file/source/job/role fields as the first approved `body-batch1/manifest.json`, plus:
...
The user authorized continuing body publication on 2026-09-23 and later requested outward media releases of at least twenty papers per batch. Human-reviewed packets should therefore accumulate at least twenty current-corpus papers before a normal publication release when sufficient backlog exists; the immutable approval files may still be split internally into five-paper/thirty-image envelopes. Prioritize recent papers and JACS among otherwise equal candidates, then waiting backlog. This media maintenance is not a replacement literature task and is not a claim that the assistant watches the desktop or repairs code continuously.
...
A lightweight repository schedule checks every five minutes. It normally invokes the heavy Pages build when twenty eligible articles are ready. If fewer than twenty remain, it waits while the eligible set is changing and flushes the tail after fifteen minutes of no new qualifying evidence. One build considers at most twenty-five new articles and at most 250 new body images; the current ten-thumbnail per-card limit remains enforced. Before any new body image is written into the static output, the same build must already contain a non-fallback official TOC for that DOI. Because `merge-local-captures.mjs` runs before the body publication step, a freshly captured official TOC and its validated body figures can become public in the same Pages deployment. Articles still missing an official TOC remain held rather than receiving body-only publication in that batch. The unchanged `literature_authorization` job remains mandatory. The repository job is not a new ChatGPT monitoring task; GitHub scheduling may still be delayed.
