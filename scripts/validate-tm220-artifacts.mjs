...
assert.ok(loader.includes("const loaderVersion = '2.2.21';")&&loader.includes('// @grant        GM_listValues'));
if(mode!=='source'){
 const bridge=fs.readFileSync(root+'/gallery-vpn-bridge.user.js','utf8');
 assert.ok(bridge.includes('// @version      2.2.21')&&bridge.includes("var VERSION = '6.2.20';"));
 ...
 console.log('TM220_RELEASE_ARTIFACT '+JSON.stringify({bridgeVersion:'2.2.21',captureVersion:'6.2.20',completeQueue:q.articles.length,queueGeneratedAt:q.generatedAt,mode:'verified-staging'}));
}else console.log('TM220_SOURCE_CONTRACT_OK');

assert.ok(src.includes("var CONTROLLER_REVISION = '2.2.21';")&&src.includes('previous_task_tab_not_closed')&&src.includes('requestControllerStart'),'Window guard regression');
