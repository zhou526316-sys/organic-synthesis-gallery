...
assert.ok(loader.includes("const loaderVersion = '2.2.33';")&&loader.includes('// @grant        GM_listValues'));
if(mode!=='source'){
 const bridge=fs.readFileSync(root+'/gallery-vpn-bridge.user.js','utf8');
 assert.ok(bridge.includes('// @version      2.2.33')&&bridge.includes("var VERSION = '6.2.20';"));
 ...
 console.log('TM220_RELEASE_ARTIFACT '+JSON.stringify({bridgeVersion:'2.2.33',captureVersion:'6.2.20',completeQueue:q.articles.length,queueGeneratedAt:q.generatedAt,mode:'verified-staging'}));
