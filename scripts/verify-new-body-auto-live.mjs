...
const result={checkedAt:new Date().toISOString(),readOnly:true,productionWrites:0,files:[],cards:[],browserRenderingVerified:false,individualSemanticReview:false};
try{
 ...
 try{
  ...
  const dois=[...new Set([...status.added.map(x=>x.doi),...snapshot.items.map(x=>x.record.doi)])].slice(0,5);
...
finally{await writeFile(out+'/acceptance.json',JSON.stringify(result,null,2));console.log('NEW_BODY_AUTO_LIVE '+JSON.stringify({result:result.result,files:result.files.length,cards:result.cards.length,browserRenderingVerified:result.browserRenderingVerified,error:result.error}));}
