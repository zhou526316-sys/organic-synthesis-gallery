...
if(directory==='dist'){
 ...
 for(const needle of ['// @version      2.2.21',"var VERSION = '6.2.21';",'// @grant        GM_getTab','// @grant        GM_saveTab','// @grant        GM_listValues','organicGalleryCloudflareBridgeWriteToken','__OSG_TOC_BROWSER_MAINLINE__','启动夜间连续抓取（TOC＋正文图）','// @updateURL    https://zhou526316-sys.github.io/organic-synthesis-gallery/gallery-vpn-bridge.user.js'])expect(packed,needle);
 assert.ok(!/\beval\s*\(/.test(packed));
}else{
 const loader=fs.readFileSync('cloudflare/scripts/build-bridge-loader.mjs','utf8');expect(loader,"loaderVersion = '2.2.21'");expect(loader,'GM_getTab');
...
console.log('TM_RELEASE_CONTRACT '+JSON.stringify({bridge:'2.2.21',core:'6.2.21',directory,articles:queue.articles.length,paired:true,nightResume:true,quarantineEpoch:queue.mediaGeneration}));
