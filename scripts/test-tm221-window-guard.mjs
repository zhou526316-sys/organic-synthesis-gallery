...
function harness(text=source,opt={}) {
  ...
  const c={Date:Clock,URL,Set,Map,Promise,crypto:{randomUUID},console,Math,Number,String,Array,Object,RegExp,
    ...
    __getJson:async url=>{await opt.onFetch?.(url,ctx,menus,store);return url.includes('capture-capabilities')?{captureVersion:'6.2.20',mediaGeneration:1790082000000,mode:'verified-staging',evidenceSchemaVersion:'article-evidence-v2',evidenceCaptureMinControllerRevision:'2.2.33'}:url.includes('toc-demand')?{generatedAt:'2026-09-22T16:30:47.570Z',mediaGeneration:1790082000000}:{items:{}};},
...
await test('cannot confirm closed tab skips the lifecycle fault and continues later DOI',async()=>{
 const h=harness(source,{neverClose:true});await h.run();const first=h.summary();await h.run();
 assert.equal(h.opened.length,20);assert.equal(first.results.length,20);assert.ok(!first.stopReason);assert.equal(h.timers.size,0);
});
...
await test('legacy viewport collectors are guarded by packaging',()=>{assert.ok(loader.includes('legacy_runtime_media_disabled'));for(const n of ['function queueDoi','function pump','function scan'])assert.ok(loader.includes(n));assert.ok(loader.includes("const loaderVersion = '2.2.33';"));});
