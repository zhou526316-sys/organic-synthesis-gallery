...
const store={'osg-toc-v6:active-job':{doi,jobId:nonce,captureVersion:'6.2.21',publisher:'acs',mediaNeed:'toc+figures',captureToc:true,startedAt:new Date().toISOString()},'osg-toc-v6:write-token':'fixture-only'};
...
try{
 ...
 rejected=false;try{await page.evaluate(({doi,nonce})=>__nightTest.assertBoundCaptureJob({doi,jobId:nonce,captureVersion:'6.2.21'}),{doi,nonce});}catch(e){rejected=String(e).includes('capture_job_stale_or_unbound');}
