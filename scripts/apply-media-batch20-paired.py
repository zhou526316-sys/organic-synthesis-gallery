...
export function selectArticleBatch(rows,media,papers,policy,{readyOverride=null,minimum=policy.minNewArticles}={}){
  ...
  return {rows:dois.length>=minimum?selected:[],dois:dois.length>=minimum?dois:[],readyArticles:dois.length,minimum};
}
"""
...
new="""async function configuration(root){
  const policy=JSON.parse(await readFile(path.join(root,'audit/media-auto-policy.json'),'utf8'));
  requireBody(policy.schemaVersion===1&&policy.policyId===POLICY_ID&&policy.minNewArticles===20&&policy.maxNewArticles>=20&&policy.maxNewArticles<=25&&policy.maxNewImages>=20&&policy.maxNewImages<=150&&policy.maxFiguresPerCard<=10&&policy.requireOfficialToc===true,'auto_invalid_configuration');"""
s=once(s,old,new)
...
new="""  const {policy,holds,papers,rows}=await pendingNewRows({root,inputs,now});
  const mediaPath=path.join(root,'public/media-index.json'),media=JSON.parse(await readFile(mediaPath,'utf8'));media.items||={};
  const minimum=options.testMode===true?1:policy.minNewArticles;
  const selection=selectArticleBatch(rows,media,papers,policy,{minimum});
...
new="""    const inputs=await readLiveInputs(),pending=await pendingNewRows({inputs});
    const selection=selectArticleBatch(pending.rows,inputs.live,pending.papers,pending.policy,{readyOverride:pending.tocReady,minimum:pending.policy.minNewArticles});
    const changed=selection.dois.length>=pending.policy.minNewArticles;
    console.log('NEW_BODY_AUTO_PENDING '+JSON.stringify({images:selection.rows.length,articles:selection.dois.length,minimum:pending.policy.minNewArticles,changed,stageError:inputs.stageError,localCaptureError:inputs.localCaptureError}));
...
# Five-minute inspection; deployment remains gated by >=20 ready articles.
p=Path('.github/workflows/new-body-continuous.yml');s=p.read_text()
s=s.replace("    - cron: '7,22,37,52 * * * *'","    - cron: '*/5 * * * *'",1)
s=s.replace('      - name: Read stored metadata and current publication ledger only','      - name: Read stored TOC/body metadata; publish only when at least twenty paired articles are ready',1)
save(p,s)
