...
export class GalleryUserShell extends HTMLElement {
  ...
  private positionPanel(): void {
    ...
    const gap = 7;
    const margin = 8;
    const below = Math.max(0, window.innerHeight - triggerRect.bottom - gap - margin);
    const above = Math.max(0, triggerRect.top - gap - margin);
    const openBelow = below >= Math.min(initialRect.height, 260) || below >= above;
    const available = openBelow ? below : above;
    const cap = this.tab === 'settings' ? 820 : 680;
    panel.style.maxHeight = `${Math.max(140, Math.min(cap, available))}px`;
    const fittedRect = panel.getBoundingClientRect();
    const top = openBelow ? triggerRect.bottom + gap : triggerRect.top - fittedRect.height - gap;
