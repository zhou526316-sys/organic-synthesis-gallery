...
function styleRow(style: StyleDef, prefix: string, label: string): string {
  const preview = style.imageData
    ? statusImageTag(style, 'style-image', label)
    : `<span>●</span>`;
...
export class GalleryUserShell extends HTMLElement {
  ...
  private render(): void {
    this.shadow.innerHTML = `<style>
      :host{position:relative;display:inline-flex;flex:0 0 auto;font:12px/1.45 Inter,system-ui,sans-serif;color:#172033}*{box-sizing:border-box}button,input,select{font:inherit}button{cursor:pointer}.trigger{min-height:34px;padding:6px 11px;border:1px solid #d7deea;border-radius:10px;background:#fff;color:#334155;font-weight:700}.trigger:hover{border-color:#9fb7f7;color:#3159bd}.panel{position:fixed;right:auto;top:8px;z-index:10010;width:min(780px,calc(100vw - 28px));height:min(760px,calc(100dvh - 16px));max-height:none;min-height:0;display:grid;grid-template-columns:148px minmax(0,1fr);grid-template-rows:minmax(0,1fr);overflow:hidden;border:1px solid #dfe5ef;border-radius:16px;background:#fff;box-shadow:0 20px 60px rgba(15,23,42,.2)}.nav{min-height:0;padding:12px;border-right:1px solid #edf0f4;background:#fafbfc}.nav button{width:100%;padding:9px;border:0;border-radius:9px;background:transparent;text-align:left;color:#475467}.nav button.active{background:#eef3ff;color:#3159bd;font-weight:750}.content{min-height:0;padding:16px;overflow-x:auto;overflow-y:scroll;scrollbar-gutter:stable;scrollbar-width:auto;scrollbar-color:#98a2b3 #eef1f5;overscroll-behavior:contain;-webkit-overflow-scrolling:touch}.content::-webkit-scrollbar{width:12px;height:10px}.content::-webkit-scrollbar-track{background:#eef1f5;border-radius:999px}.content::-webkit-scrollbar-thumb{background:#98a2b3;border:3px solid #eef1f5;border-radius:999px}.content::-webkit-scrollbar-thumb:hover{background:#667085}.head{display:flex;justify-content:space-between;gap:12px;margin-bottom:12px}.head h3{margin:0;font-size:17px}.close{border:0;border-radius:8px;width:28px;height:28px;background:#f2f4f7}.item{display:grid;gap:3px;padding:10px 0;border-top:1px solid #edf0f4}.item a{color:#243044;text-decoration:none;font-weight:700}.item small,.help{color:#8a93a3;font-size:10px}.empty{padding:18px 0;color:#8a93a3}.row{display:flex;gap:7px;align-items:center;flex-wrap:wrap}.secondary,.link{padding:7px 9px;border:1px solid #dfe5ef;border-radius:9px;background:#fff;color:#475467}.link{border:0;padding:4px;background:transparent;color:#3159bd}.danger{color:#b42318}.section{padding:12px 0;border-top:1px solid #edf0f4}.section h4{margin:0 0 8px}.manage{display:grid;gap:8px}.manage-row{padding:9px;border:1px solid #e5e9f0;border-radius:11px}.manage-row>.top{display:flex;align-items:center;gap:7px;flex-wrap:wrap}.manage-row input[type=text]{min-width:140px;flex:1}.manage-row input[type=text],.manage-row select,.amount,.email{border:1px solid #d7deea;border-radius:8px;padding:6px}.style-row{display:grid;grid-template-columns:auto minmax(36px,1fr) minmax(82px,auto) minmax(112px,auto) minmax(130px,1fr) auto;align-items:center;gap:8px;padding:9px 0;border-top:1px dashed #e5e9f0}.style-preview{display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;background:var(--u-color);color:var(--u-text);overflow:hidden}.style-preview img{width:100%;height:100%;object-fit:contain}.style-preview span{font-size:12px}.style-row label{display:flex;align-items:center;gap:4px;font-size:10px}.style-row input[type=color]{width:30px;height:26px;border:0;background:transparent;padding:0}.upload input{width:min(150px,100%);min-width:0;font-size:9px}.provider{display:grid;gap:7px}.provider button{padding:9px;border:1px solid #e1e6ee;border-radius:10px;background:#fff;text-align:left}.provider button:disabled{cursor:not-allowed;opacity:.52}.provider .ok{color:#27845b}.provider .off{color:#8a93a3}.amounts{display:flex;gap:6px;flex-wrap:wrap}.amounts button{padding:7px 10px;border:1px solid #d7deea;border-radius:9px;background:#fff}.notice{margin-top:10px;padding:9px;border-radius:9px;background:#f8fafc;color:#667085;font-size:10px}.user-card{display:flex;align-items:center;gap:10px;padding:10px;border:1px solid #e5e9f0;border-radius:11px}.avatar{width:38px;height:38px;border-radius:50%;object-fit:cover;background:#eef1f5}.pay-result{margin-top:10px;padding:10px;border:1px solid #e5e9f0;border-radius:10px;overflow-wrap:anywhere}.pay-result a{color:#3159bd}.email{min-width:220px;flex:1}.auth-tabs{display:flex;gap:6px;margin-bottom:10px}.auth-tabs button{flex:1;padding:8px;border:1px solid #dfe5ef;border-radius:9px;background:#fff;color:#475467}.auth-tabs button.active{border-color:#9fb7f7;background:#eef3ff;color:#3159bd;font-weight:750}.auth-form{display:grid;gap:8px}.auth-form input{width:100%;border:1px solid #d7deea;border-radius:9px;padding:9px}.auth-form .code-input{font-size:18px;letter-spacing:6px;text-align:center;font-weight:750}.account-grid{display:grid;gap:9px;margin-top:10px}.verified{color:#27845b;font-weight:700}.unverified{color:#b54708;font-weight:700}.primary{padding:9px 11px;border:1px solid #3159bd;border-radius:9px;background:#3159bd;color:#fff;font-weight:750}.primary:disabled{cursor:wait;opacity:.7}.auth-notice{margin-top:0;font-size:11px;color:#344054;background:#f2f4f7}.qr-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:10px}.qr-card{display:grid;gap:7px;padding:10px;border:1px solid #e5e9f0;border-radius:12px;background:#fff;text-align:center}.qr-card strong{font-size:12px}.qr-card a{display:block;border-radius:10px;overflow:hidden;background:#fff}.qr-card img{display:block;width:100%;aspect-ratio:1;object-fit:contain}.support-note{margin:0;color:#667085;font-size:11px;line-height:1.6}
      @media(max-width:680px){.panel{position:fixed;right:auto;top:8px;inset:auto;width:min(520px,calc(100vw - 24px));height:min(620px,calc(100dvh - 16px));max-height:none;min-height:0;grid-template-columns:1fr;grid-template-rows:auto minmax(0,1fr);overflow:hidden;border-radius:16px}.nav{display:flex;overflow-x:auto;overflow-y:hidden;border-right:0;border-bottom:1px solid #edf0f4;padding:7px}.nav button{width:auto;white-space:nowrap}.content{min-height:0;overflow-x:hidden;overflow-y:scroll;scrollbar-gutter:stable;scrollbar-width:auto;scrollbar-color:#98a2b3 #eef1f5;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;padding:12px 12px 28px}.style-row{grid-template-columns:1fr 1fr}.trigger{min-height:32px;padding:5px 9px}}
      .style-preview.status-original-preview{width:96px;height:40px;clip-path:none;transform:none;border-radius:0;background:transparent;overflow:visible}
  ...
  private async action(action: string): Promise<void> {
    if (action.startsWith('crop-image:') || action.startsWith('restore-image:')) {
    ...
    if (action.startsWith('view-image:')) { const target = this.styleTarget(action.slice(11)); if (target) await viewStatusImage(target); return; }
