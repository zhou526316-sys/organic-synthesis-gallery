...
export class GalleryPaperActions extends HTMLElement {
  ...
  private statusVisual(status: NonNullable<ReturnType<typeof store.status>>, className: string, imageReplacesLabel = false): string {
    const label = statusLabel(status, this.language);
    if (status.style.imageData && imageReplacesLabel) {
      return `<span class='${className} status-custom-image shape-${status.style.shape}' title='${escapeHtml(label)}' aria-label='${escapeHtml(label)}'><img class='status-image' src='${escapeHtml(status.style.imageData)}' alt='${escapeHtml(label)}'></span>`;
    }
    const image = status.style.imageData
      ? `<img class='status-image' src='${escapeHtml(status.style.imageData)}' alt=''>`
      : '';
    return `<span class='${className} shape-${status.style.shape}' style='background:${rgbCss(status.style.rgb)}'>${image}<span>${escapeHtml(label)}</span></span>`;
  }
  ...
  private chips(paper: PaperUserState, status: ReturnType<typeof store.status>): string {
    ...
    const values = [status ? this.statusVisual(status, 'chip status', true) : '', ...quick.map(item => `<span class='chip'>${escapeHtml(item.label)}</span>`), ...paper.tags.map(tag => `<span class='chip'>${escapeHtml(tag)}</span>`)];
