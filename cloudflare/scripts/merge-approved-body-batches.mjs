...
export function verifyApprovedBodyItem(item,bytes) {
  ...
  demand(/^audit\/media-recovery\/body-batches\/assets\/[a-f0-9]{64}\.(svg|png|webp|jpe?g)$/.test(item.assetPath),'approved_body_path_invalid');
  return ext;
}
...
export async function mergeApprovedBodyBatches(root=process.cwd()) {
  ...
  for(const name of names) {
    ...
    for(const item of batch.items) {
      demand(typeof item.assetPath==='string'&&/^audit\/media-recovery\/body-batches\/assets\/[a-f0-9]{64}\.(svg|png|webp|jpe?g)$/.test(item.assetPath),'approved_body_path_invalid');
