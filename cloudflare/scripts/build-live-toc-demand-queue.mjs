...
async function loadPapers() {
  ...
  const [total, manual, audit, curated, automation, rolling, localLiterature, remoteLiterature] = await Promise.all([
    ...
    readJson('literature-supplement.json'),
  ...
  const all = [].concat(
    ...
    automation?.papers || [],
    rolling?.papers || [],
    localLiterature?.papers || [],
    remoteLiterature?.papers || []
  );
