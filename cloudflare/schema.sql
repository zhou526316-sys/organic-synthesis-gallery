...
-- Durable GPT-reviewed summary queue. Raw evidence/draft/audit artifacts remain
-- private in R2; D1 stores only hashes and queue/lease/retry metadata.
CREATE TABLE IF NOT EXISTS article_summary_jobs (
  ...
  state TEXT NOT NULL DEFAULT 'queued' CHECK (state IN (
    'queued','leased','draft_ready','audit_running','audit_passed',
    'retry_wait','needs_manual_review','published','blocked'
  )),
  priority INTEGER NOT NULL DEFAULT 0,
  attempts INTEGER NOT NULL DEFAULT 0,
  next_retry_at INTEGER NOT NULL DEFAULT 0,
